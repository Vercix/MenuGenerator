...
export default function Page() {
  ...
  function moveItem(index: number, amount: number) {
    if (index >= 0 && index < menuItems.length) {
      if(amount > 0 && index == menuItems.length - 1){
        return
      }
      if(amount < 0 && index == 0){
        return
      ...
      let moveToIndex : number = index + amount;
      moveToIndex = Math.min(Math.max(moveToIndex, 0) , menuItems.length - 1)
      
      let el : IMenuItem = menuItems[index]
      menuItems[index] = menuItems[moveToIndex]
      menuItems[moveToIndex] = el
                  ...
                  <Box >
                    <Paper sx={{ padding: 1 }} variant="outlined">
                      <Grid container spacing={1} columns={16}>
                        ...
                        <Grid size={{ xs: 8, md: 2 }}>
                          <Button onClick={() => moveItem(index, -1)}>Move Up</Button>
                          <Button onClick={() => moveItem(index, 1)}>Move Down</Button>
